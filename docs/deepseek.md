...
模型调用日志只新增数字设置、耗时和异常类型，不输出请求正文、思考字段、密钥、服务地址或服务商原始错误。它使用实际 SDK 对象的设置，显示：
...
遇到超时，请反馈最后一轮的 `[研究员]`、`[模型调用]` 日志和页面错误提示。首次排查先保持现有配置，以便区分连接问题与等待响应问题；这些诊断不改变超时、重试次数或模型思考行为，也不代表网络问题已修复。
